-- Match shared SRS rounding and minimum intervals for authenticated reviews.
-- Keep the original migration immutable; this replaces only the RPC body.
...
DECLARE
  ...
  v_next_interval_raw DOUBLE PRECISION;
...
BEGIN
  ...
  v_next_interval_raw := v_word.interval_days;
  ...
  CASE p_assessment
    WHEN 'again' THEN
      ...
      v_next_repetition := 0;
      v_next_interval_raw := 0;
    WHEN 'hard' THEN
      v_next_easiness := GREATEST(1.3, v_word.easiness_factor - 0.15);
      v_next_repetition := v_word.repetition_count + 1;
      v_next_interval_raw := CASE
        WHEN v_word.interval_days = 0 THEN 1
        ELSE v_word.interval_days * 1.2::DOUBLE PRECISION
      END;
    WHEN 'good' THEN
      v_next_repetition := v_word.repetition_count + 1;
      v_next_interval_raw := CASE v_next_repetition
        WHEN 1 THEN 1
        WHEN 2 THEN 6
        ELSE v_word.interval_days * v_word.easiness_factor
      END;
    WHEN 'easy' THEN
      ...
      v_next_interval_raw := CASE v_next_repetition
        WHEN 1 THEN 4
        WHEN 2 THEN 10
        ELSE v_word.interval_days * v_word.easiness_factor * 1.3::DOUBLE PRECISION
      END;
  END CASE;

  -- PostgreSQL ROUND(float8) uses ties-to-even on common platforms. Match
  -- JavaScript Math.round for nonnegative intervals using the fractional part.
  -- Avoid rounding the product to NUMERIC or adding 0.5 before FLOOR: either
  -- can turn a binary float just below a half into an incorrect upward tie.
  v_next_interval := CASE
    WHEN p_assessment = 'again' THEN 0
    ELSE GREATEST(
      1,
      FLOOR(v_next_interval_raw)::INTEGER
        + CASE WHEN v_next_interval_raw - FLOOR(v_next_interval_raw) >= 0.5
          THEN 1 ELSE 0 END
    )
  END;
