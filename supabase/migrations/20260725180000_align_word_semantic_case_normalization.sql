-- Align remote semantic uniqueness with the case-insensitive SQLite key.
-- Existing collisions require an explicit product decision and must not be
...
BEGIN
  ...
  FROM (
    ...
    GROUP BY
      ...
      COALESCE(article, '')
    HAVING count(*) > 1
  ) AS mixed_case_collisions;

  IF collision_count > 0 THEN
    RAISE EXCEPTION
      'Cannot enforce case-insensitive word uniqueness: % active semantic key collision(s) require manual resolution',
      collision_count
      USING HINT =
        'Resolve active words that differ only by dutch_lemma case, then retry the migration.';
  END IF;
END;
$$;
...
COMMENT ON INDEX public.idx_words_semantic_unique IS
  'Ensures case-insensitive semantic uniqueness among active words only.';

-- Keep shared imports conflict-safe against the expression-based partial
-- unique index. PostgreSQL requires the conflict target and predicate to match.
  ...
  LOOP
    ...
    IF v_inserted_word.word_id IS NULL THEN
      ...
      WHERE w.user_id = auth.uid()
        AND lower(w.dutch_lemma) = v_normalized_lemma
...
COMMENT ON FUNCTION private.import_words_to_collection IS
  'Imports shared words with case-insensitive uniqueness among active rows.';
