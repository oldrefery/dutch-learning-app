...
if [[ "$PLATFORM" == "ios" || "$PLATFORM" == "both" ]]; then
    ...
    echo -e "${YELLOW}Running: npx eas-cli@latest build --platform ios --profile production --local --output builds/app-${NEW_VERSION}-${NEW_BUILD}.ipa${NC}"

    if EAS_SKIP_AUTO_FINGERPRINT=1 NODE_ENV=production npx eas-cli@latest build --platform ios --profile production --local --output "builds/app-${NEW_VERSION}-${NEW_BUILD}.ipa" --non-interactive --json > builds/ios-build-metadata.json; then
...
if [[ "$PLATFORM" == "android" || "$PLATFORM" == "both" ]]; then
    ...
    echo -e "${YELLOW}Running: npx eas-cli@latest build --platform android --profile production --local --output builds/app-${NEW_VERSION}-${NEW_BUILD}.aab${NC}"

    if EAS_SKIP_AUTO_FINGERPRINT=1 NODE_ENV=production npx eas-cli@latest build --platform android --profile production --local --output "builds/app-${NEW_VERSION}-${NEW_BUILD}.aab" --non-interactive --json > builds/android-build-metadata.json; then
...
if [[ "$SUBMIT" == "true" ]]; then
    ...
    if [[ "$PLATFORM" == "ios" || "$PLATFORM" == "both" ]]; then
        ...
        echo -e "${YELLOW}Running: npx eas-cli@latest submit -p ios --path ${IOS_FILE}${NC}"
        npx eas-cli@latest submit -p ios --path "$IOS_FILE"
    ...
    if [[ "$PLATFORM" == "android" || "$PLATFORM" == "both" ]]; then
        ...
        echo -e "${YELLOW}Running: npx eas-cli@latest submit -p android --path ${ANDROID_FILE}${NC}"
        npx eas-cli@latest submit -p android --path "$ANDROID_FILE"
