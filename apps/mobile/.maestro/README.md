...
2. Environment variables are configured in `apps/mobile/.maestro/.maestro.env` or `apps/mobile/.env`:
...
Runs all test flows in `apps/mobile/.maestro/` in alphabetical order.
...
maestro test apps/mobile/.maestro/ --verbose  # Run all with verbose output
```
...
1. Read `MAESTRO_TEST_EMAIL` and `MAESTRO_TEST_PASSWORD` from `apps/mobile/.maestro/.maestro.env` or `apps/mobile/.env`
...
```bash
# Copy example files
cp apps/mobile/.maestro/.maestro.env.example apps/mobile/.maestro/.maestro.env
cp apps/mobile/.maestro/config.yaml.example apps/mobile/.maestro/config.yaml
