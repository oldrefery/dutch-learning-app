...
async function requestGoogleOAuthUrl(redirectTo: string): Promise<string> {
...
export async function initiateGoogleOAuth(): Promise<WebBrowser.WebBrowserAuthSessionResult> {
  ...
  try {
    const oauthUrl = await requestGoogleOAuthUrl(redirectTo)
...
async function setSessionFromOAuthUrl(url: string): Promise<void> {
